...
public class UserController extends BaseController {
	...
	public Json login(User user, HttpSession session, HttpServletRequest request) {
		...
		if (u != null) {
			...
			bugService.add(user.getName()+" 登陆成功", sessionInfo);
	...
	@RequestMapping("/download")
	 public void download(HttpServletRequest request,  
	        ...
	        String rootpath = request.getSession().getServletContext().getRealPath(  
	                "/");  
	        String fileName ="usingDocument.docx";  
	        try {  
	...
	public Json add(User user, HttpSession session) {
		...
		try {
	        ...
	        bugService.add("新增用户---"+user.getName(), sessionInfo);
			j.setSuccess(true);
			j.setMsg("添加成功！");
			j.setObj(user);
		} catch (Exception e) {
	...
	public Json edit(User user, HttpSession session) {
		...
		try {
			userService.edit(user);
			  bugService.add("修改用户---"+user.getName(), sessionInfo);
			j.setSuccess(true);
			j.setMsg("编辑成功！");
			j.setObj(user);
		} catch (Exception e) {
	...
	public Json delete(String id, HttpSession session) {
		...
		if (id != null && !id.equalsIgnoreCase(sessionInfo.getId())) {// 不能删除自己
			userService.delete(id);
			 bugService.add("删除用户---"+id, sessionInfo);
		}
		j.setMsg("删除成功！");
		j.setSuccess(true);
		return j;
