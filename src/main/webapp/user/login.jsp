 ...
 */
	...
	function loginFun() {
		if (layout_west_tree0) {//当west功能菜单树加载成功后再执行登录
				...
				$.post('${pageContext.request.contextPath}/userController/login', form.serialize(), function(result) {
					if (result.success) {
						if (!layout_west_tree_url) {
							...
							layout_west_tree0.tree({
								url : '${pageContext.request.contextPath}/resourceController/tree?type=0',
							 ...
							 $('#layout_west_tree2').tree({
								url : '${pageContext.request.contextPath}/resourceController/tree?type=2',
								onBeforeLoad : function(node, param) {
									parent.$.messager.progress({
										title : '提示',
										text : '数据处理中，请稍后....'
										...
										var node = layout_west_tree2.tree('getRoots');
										  ...
										  var nodechild=layout_west_tree2.tree('getChildren',node[i].target);
										   for(var j=0;j<nodechild.length;j++){
												 ...
												 try{
													    if(!contains(result.obj.groupn,nodechild[j].text))
						...
						if(!contains(result.obj.resourceList.toString(),'/resourceController/addPage'))
							$('#addresource').css("display", "none");
						if(!contains(result.obj.resourceList.toString(),'/resourceController/editPage'))
							$('#editresource').css("display", "none");
						if(!contains(result.obj.resourceList.toString(),'/resourceController/delete'))
							$('#deresource').css("display", "none");
					} else {
