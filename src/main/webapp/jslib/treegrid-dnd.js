...
//						e.data.offsetHeight = 0;
				...
				//==================================
                 //做自己的逻辑处理
                var src  = $('#datagrid_depttree').treegrid('find', $(source).attr('node-id'));
                //var dest = $('#datagrid_depttree').treegrid('find', $(this).attr('node-id'));
                var paramNewParent = src._parentId;
                var paramDeptid = src.deptid;
                $.ajax({
                   url: '../action/updateDeptByDrog.a',
                    dataType: 'json',
                    type:'post',
                    data: {
                        newParent: paramNewParent,
                        deptid:paramDeptid
                    },
                    success:function(data){
                        ...
                        {
                        	//alert("pbj");
                          //  $('#datagrid_depttree').treegrid('reload');    //重新加载treegrid        
                       }
                    }
