...
<script type="text/javascript">
	$(function() {
		...
		$('#form').form({
			url : '${pageContext.request.contextPath}/caseController/edit',
			onSubmit : function() {
	...
	<div data-options="region:'center',border:false" title="" style="overflow: hidden;">
		<form id="form" method="post">
			<table class="table table-hover table-condensed">
				...
				<tr>
					<th>测试项目</th>
					<td><input name="name" id="name" type="text" placeholder="请输入测试项目" data-options="required:true" class="easyui-validatebox span2" value="${case.name}"></td>
					<th>执行级</th>
				...
				<tr>
					<th>排序</th>
					<td><input name="seq" value="${case.seq}" class="easyui-numberspinner" style="width: 140px; height: 29px;" required="required" data-options="editable:false,min:100"></td>
					<th>上级目录</th>
				...
				<tr>
					<th>前置条件</th>
					<td colspan="3"><textarea name="pre" rows="" cols="" class="span5">${case.pre}</textarea></td>
				</tr>
				<tr>
					<th>执行步骤</th>
					<td colspan="3"><textarea name="step" rows="" cols="" style="width: 367px; height: 70px;" class="span5">${case.step}</textarea></td>
				</tr>
					<tr>
					<th>预期结果</th>
					<td colspan="3"><textarea name="expire" rows="" cols="" style="width: 367px; height: 70px;" class="span5">${case.expire}</textarea></td>
				</tr>
				<tr>
					<th>备注</th>
					<td colspan="3"><textarea name="remark" rows="" cols="" class="span5">${case.remark}</textarea></td>
				</tr>
			</table>
		</form>
	</div>
